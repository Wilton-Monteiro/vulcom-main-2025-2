...
export default function CarForm() {
  ...
  */
  const formDefaults = {
    ...
    plates: '',
    selling_date: null,
    selling_price: '', // Iniciar como string vazia para o campo
    customer_id: ''
  }
  ...
  // O estado 'imported' é inicializado com base no 'car.imported'
  // Isso é crucial para a edição
  const imported = car.imported
  ...
  async function handleFormSubmit(event) {
    ...
    try {
      ...
      // Cópia do carro para manipulação
      const carToValidate = { ...car }

      // Ajuste para valores que podem ser nulos ou números
      if(carToValidate.selling_price === '' || carToValidate.selling_price === null) {
        carToValidate.selling_price = null
      } else {
        // Converte o preço para número se não for nulo
        carToValidate.selling_price = Number(carToValidate.selling_price)
      }
      
      if(carToValidate.selling_date === '') {
        carToValidate.selling_date = null
      }

      // O ano é salvo como número
  ...
  async function loadData() {
    ...
    try {
      ...
      if(params.id) {
        ...
        // Garante que selling_price seja string vazia se for null
            ...
            <FormControlLabel
              control={
                <Checkbox
                  name='imported'
                  variant='filled'
                  checked={car.imported} // Controlado pelo estado 'car'
